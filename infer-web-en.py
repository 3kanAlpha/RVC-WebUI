...
def train_index(exp_dir1):
    ...
    if(os.path.exists(feature_dir)==False):return "Perform feature extraction first!"
    listdir_res=list(os.listdir(feature_dir))
    if(len(listdir_res)==0):return "Perform feature extraction first!"
    npys = []
...
def train1key(exp_dir1, sr2, if_f0_3, trainset_dir4, spk_id5, gpus6, np7, f0method8, save_epoch10, total_epoch11, batch_size12, if_save_latest13, pretrained_G14, pretrained_D15, gpus16, if_cache_gpu17):
    ...
    else:yield get_info_str("step2a: No pitch extraction required")
    #######step2b:提取特征
    yield get_info_str("step2b: Feature extraction in progress")
    gpus=gpus16.split("-")
    leng=len(gpus)
    ps=[]
    for idx,n_g in enumerate(gpus):
    ...
    index.add(big_npy)
    faiss.write_index(index, '%s/added_IVF%s_Flat_nprobe_%s.index'%(exp_dir,n_ivf,index_ivf.nprobe))
    yield get_info_str("Index is built successfully, added_IVF%s_Flat_nprobe_%s.index"%(n_ivf,index_ivf.nprobe))
    yield get_info_str("The whole process is done!")
...
with gr.Blocks() as app:
    gr.Markdown(value="""
        This software is open source under the MIT protocol, the author does not have any control over the software, the use of the software, the dissemination of the software exported sound is fully responsible for their own.<br>
        If you don't agree, you may not use or reference any code or files within the package. See "AGREEMENT-LICENSE.txt" for details.
        """)
    with gr.Tabs():
        with gr.TabItem("Inference"):
            with gr.Row():
                sid0 = gr.Dropdown(label="Speaker", choices=sorted(names))
                ...
                clean_button = gr.Button("Unload model", variant="primary")
                spk_item = gr.Slider(minimum=0, maximum=2333, step=1, label='Choose speaker id', value=0, visible=False, interactive=True)
                clean_button.click(
                    fn=clean,
                    inputs=[],
                    outputs=[sid0]
            ...
            with gr.Group():
                gr.Markdown(value="""
                    +12 key is recommended for male to female conversion, and -12 key is recommended for female to male conversion.
                    """)
                with gr.Row():
                    with gr.Column():
                        vc_transform0 = gr.Number(label="Pitch (Integer, semitone steps, 12 for 1 octave)", value=0)
                        input_audio0 = gr.Textbox(label="Enter the path to the audio file (the default value is a correct format example)",value="E:\codes\py39\\vits_vc_gpu_train\\todo-songs\冬之花clip1.wav")
                        f0method0=gr.Radio(label="Choose the pitch extraction algorithm: pm is fast, harvest is for better quality, but slow", choices=["pm","harvest"],value="pm", interactive=True)
                    with gr.Column():
                        file_index1 = gr.Textbox(label="Path to feature retrieval library",value="E:\codes\py39\\vits_vc_gpu_train\logs\mi-test-1key\\added_IVF677_Flat_nprobe_7.index", interactive=True)
                        file_big_npy1 = gr.Textbox(label="Path to feature file",value="E:\codes\py39\\vits_vc_gpu_train\logs\mi-test-1key\\total_fea.npy", interactive=True)
                        index_rate1 =  gr.Slider(minimum=0, maximum=1,label='检索特征占比', value=1,interactive=True)
                    f0_file = gr.File(label="F0 curve file (optional), one pitch per line, instead of default F0 and up/down")
                    but0=gr.Button("Convert", variant="primary")
                    with gr.Column():
                        vc_output1 = gr.Textbox(label="Log")
            ...
            with gr.Group():
                ...
                with gr.Row():
                    with gr.Column():
                        vc_transform1 = gr.Number(label="Pitch (Integer, semitone steps, 12 for 1 octave)", value=0)
                        opt_input = gr.Textbox(label="Enter the path to output directory", value="opt")
                        f0method1=gr.Radio(label="Choose the pitch extraction algorithm: pm is fast, harvest is for better quality, but slow", choices=["pm","harvest"], value="pm", interactive=True)
                    with gr.Column():
                        file_index2 = gr.Textbox(label="Path to feature retrieval library",value="E:\codes\py39\\vits_vc_gpu_train\logs\mi-test-1key\\added_IVF677_Flat_nprobe_7.index", interactive=True)
                        file_big_npy2 = gr.Textbox(label="Path to feature file",value="E:\codes\py39\\vits_vc_gpu_train\logs\mi-test-1key\\total_fea.npy", interactive=True)
                        index_rate2 =  gr.Slider(minimum=0, maximum=1,label='检索特征占比', value=1,interactive=True)
                    with gr.Column():
                        dir_input = gr.Textbox(label="Enter the path of the audio folder",value="E:\codes\py39\\vits_vc_gpu_train\\todo-songs")
                        inputs = gr.File(file_count="multiple", label="Input audio files, the directory is read in priority")
                    but1=gr.Button("Convert", variant="primary")
                    vc_output3 = gr.Textbox(label="Log")
